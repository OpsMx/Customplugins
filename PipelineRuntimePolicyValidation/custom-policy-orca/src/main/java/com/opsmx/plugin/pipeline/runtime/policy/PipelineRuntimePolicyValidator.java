...
public class PipelineRuntimePolicyValidator implements ExecutionPreprocessor, SpinnakerExtensionPoint {
	...
	public Map<String, Object> process(@Nonnull Map<String, Object> pipeline){
		...
		if (!(opaConfigProperties.isEnabled() && (opaConfigProperties.getRuntime() != null && opaConfigProperties.getRuntime().isEnabled()))) {
			logger.info("Pipeline Runtime Policy validation not enabled with OPA, returning");
		...
		try {
			...
			e.printStackTrace();
			logger.error("Communication exception for ISD at {}: {}", opaConfigProperties.getRuntimeUrl(), e.toString());
			logger.debug("End of the Policy Validation");
			throw new ValidationException(e.toString(), null);
		} catch (Exception e) {
			logger.error("Some thing wrong While processing the OPA Validation : ",e);
			logger.debug("End of the Policy Validation");
			throw new ValidationException(e.toString(), null);
		}
		logger.debug("End of the Policy Validation");
