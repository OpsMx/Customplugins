...
public class ValidationManualjudgmentPipeline {
    ...
    private String validateOPAResponse(String opaStringResponse){
        ...
        if (opaResponse.has(RESULT)) {
            ...
            if (opaResult.has(opaConfigProperties.getResultKey())) {
                ...
                if (StringUtils.isNotBlank(denyMessage)) {
                    msg = denyMessage.toString();
                }
            } else {
                msg ="There is no '" + opaConfigProperties.getResultKey() + "' field in the OPA response";
            }
        } else {
            msg = "There is no 'result' field in the OPA response";
        }
    return msg;
    }
